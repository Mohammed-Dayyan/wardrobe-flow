-- User-local calendar dates for outfit logging (align app "today" with Postgres checks)

ALTER TABLE profiles
  ADD COLUMN IF NOT EXISTS timezone TEXT NOT NULL DEFAULT 'UTC';

CREATE OR REPLACE FUNCTION user_local_date(p_timezone text)
RETURNS date
LANGUAGE sql
STABLE
AS $$
  SELECT (now() AT TIME ZONE COALESCE(NULLIF(trim(p_timezone), ''), 'UTC'))::date;
$$;
