-- Single-round-trip analytics snapshot (reuses 009 RPCs internally)
...
AS $$
  SELECT jsonb_build_object(
    ...
    'has_data', (
      ...
      OR EXISTS (SELECT 1 FROM wear_history WHERE user_id = auth.uid())
    ),
    'breakdown', (
      SELECT COALESCE(
        jsonb_agg(
...
GRANT EXECUTE ON FUNCTION get_analytics_snapshot(TEXT, INT) TO authenticated;
